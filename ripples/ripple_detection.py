...
def get_quality_metrics(
    ...
    SRP_check = [
        ...
        >= supra_ripple_band_power[
            np.where(np.array(detection_channels_ca1) == event.detection_channel)[0],
            event.peak_idx,
        ]
        * 2
        for event in candidate_events
...
def detect_ripple_events(
    ...
    inst_freq_lfp = calculate_instantaneous_frequency(
        bandpass_filter(
            lfp_det_chans,
            RIPPLE_BAND[0],
            RIPPLE_BAND[1],
            ...
            if np.logical_and(
                ...
                candidate_events.append(
                    CandidateEvent(
                        ...
                        instantaneous_frequency=inst_freq_lfp[peak_idx],
